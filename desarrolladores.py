...
def resumen(nombre,apellido,tipofirma,carpetaentrada,carpetasalida):
    ...
    if all(x.isalpha() or x.isspace() for x in nombre)==False:
            errorNombre = messagebox.showerror("Error","Ingresó un carácter no valido en el campo Nombre")
    
    if all(x.isalpha() or x.isspace() for x in apellido)==False:
            errorApellido = messagebox.showerror("Error","Ingresó un carácter no valido en el campo Apellido")
    if apellido == "":
    ...
    if nombre != "" and apellido != "" and tipofirma != "Seleccionar..." and carpetaentrada != "" and carpetasalida != "" and all(x.isalpha() or x.isspace() for x in nombre)==True and all(x.isalpha() or x.isspace() for x in apellido)==True:
        ...
        if response == 1:
            ...
            for j in range(0,len(last_page)):
                ...
                while (i<total-1):
                    bloque=bloque+1
                    i=i+1
                    if(data_image['text'][i].casefold()==a[0].casefold() or data_image['text'][i].casefold()==b[0].casefold()):
                        newdata=[0,0]
                ...
                while (i<n_boxes-1):
                    bloque=bloque+1
                    i=i+1
                    while (newy-height<data_image['top'][i]) and (data_image['top'][i]<newy):
                        newy=newy-20
                        while (newx+width>data_image['left'][i]) and (data_image['left'][i]>newx and newy-height==data_image['top'][i]):
                            newx=newx+5
            ...
            mylist = Listbox(progreso, yscrollcommand = scrollbar.set )
            for line in no_signed:
                mylist.insert(END, line)#"This is line number " + str(line))
...
Label(progreso, text="Progreso ", font=(fuente, 14)).pack()
